...
| Bible PDF ingestion | Manual file placement in `KB/` folder | Ingest Brandon's 250-page PDF into Qdrant — no backend endpoint needed |
...
| KB Search / Document Metadata / KB Versioning APIs | Agent queries its vector database (Qdrant) directly. These would serve an admin panel, not the chatbot. |
